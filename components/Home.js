...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-76sk.vercel.app/movies')
    .then(response => response.json())
    .then(data => {
      const formattedData = data.movies.map((movie) => {
        const poster = `https://image.tmdb.org/t/p/w500/${movie.poster.path}`
        let overview = movie.overview
        if (overview.length > 250) {
          overview = overview.substring(0, 250) + "..."
        }
        return {title: movie.title, poster, voteAverage: movie.vote_average, voteCount : vote_count, overview}
      })
